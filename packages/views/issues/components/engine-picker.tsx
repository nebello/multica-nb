...
/**
 * The pin as the UI reads it: the two keys, the runtime catalog they point
 * into, and the label that runtime renders as.
 *
 * Shared with the chat composer's pill, which shows the same value from a
 * different surface — the pin has one label wherever it is displayed, so the
 * pill and the issue's Engine row can never drift apart (NEB-668).
 */
export function useIssueEnginePin(issue: Pick<Issue, "metadata"> | undefined) {
  const { t } = useT("issues");
  ...
  return { pin, runtimes, pinned, runtimeLabel };
}
...
export function IssueEngineRows({
  ...
  const { pin, runtimes, pinned, runtimeLabel } = useIssueEnginePin(issue);
